...
namespace SimpleMemoryPool
{
    LYW_CODE::SimpleMemoryPool::MemoryPool::MemoryPool()
    {
        ...
        for (int iLoop = 0; iLoop < SIM_MEM_MAX_EX; iLoop++)
        {
            ...
            if (iLoop <= 10)
            {
                m_cfg[iLoop].nodeCount = 4096 / dec;

                if (m_cfg[iLoop].nodeCount < 32)
                {
                    m_cfg[iLoop].revertCount = 32;
                }
                else
                {
                    m_cfg[iLoop].revertCount = m_cfg[iLoop].nodeCount;
                }
    ...
    void LYW_CODE::SimpleMemoryPool::MemoryPool::FreeNode(Node_t * node)
    {
        ...
        if (node->block->occupyCount <= 0)
        {
            ...
            if (node->block->pre != NULL)
            {
                node->block->pre->next = node->block->next;
            }
            else
            {
                m_block = node->block;
            ...
            {
                node->block->next->pre = node->block->pre;
            }
